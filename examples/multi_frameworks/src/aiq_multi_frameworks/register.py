...
async def multi_frameworks_workflow(config: MultiFrameworksWorkflowConfig, builder: Builder):
    ...
    'Retrieve' - any question related to poems, either in English or in Chinese like 'poem about mountain' or "关于山的诗词".
    'General' - any question about weather or stock market like "weather today", "Nvidia tock price", "stock market today".
    'Research' - any other question that is not related to the above two topics.
    User query: {input}
    ...
    async def workers(state: AgentState):
        ...
        if "retrieve" in worker_choice.lower():
            ...
            #out = await rag_tool.ainvoke({"messages": [query]})
            #from langchain_core.messages import HumanMessage
            #out = await rag_tool.ainvoke({"messages": [HumanMessage(content=query)]})
            #out = await rag_tool.ainvoke({"messages": [{"role": "user", "content": query}]})
            output = out
        ...
        else: # 'research' in worker_choice.lower():
            #inputs = {"question": query}
            #output = (await research_tool.ainvoke(inputs))
            output = (await research_tool.ainvoke(query))
        #else:
